...
export const InputService = {
    async fetchApplicationInputs() {
        // Fetch the registered ePolyScat application interface from the backend so the
        // input model stays aligned with what is deployed. Input types are DataType
        // name strings ("STRING"/"URI"/...), matching the frontend, so no transform.
        const {data: settings} = await axiosInstance.get("/epolyscat_django_app/api/settings/");
        const appModuleId = settings.EPOLYSCAT.EPOLYSCAT_APPLICATION_ID;
        const {data} = await axiosInstance.get(`/api/applications/${appModuleId}/application_interface/`);
        // Backend returns inputs alphabetically; restore definition order.
        return (data.applicationInputs || []).slice().sort((a, b) => a.inputOrder - b.inputOrder);
    },
